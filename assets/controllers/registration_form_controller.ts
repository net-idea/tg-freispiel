...
/**
 * Submits the Anmeldung form via fetch and renders the JSON result inline
 * (success banner, per-field validation errors). Without JavaScript the form
 * falls back to a regular POST handled server-side.
 */
export default class extends Controller<HTMLElement> {
  ...
  async submit(event: Event): Promise<void> {
    ...
    try {
      ...
      if (result.success) {
        this.showSuccess(result.message || this.successMessageValue);
      } else {
  ...
  private showFieldErrors(errors: Record<string, string[]>): void {
    for (const [field, messages] of Object.entries(errors)) {
      ...
      const widget =
        this.formTarget.querySelector<HTMLElement>(`[name="form_registration[${field}]"]`) ??
        this.formTarget.querySelector<HTMLElement>(`[name="form_registration[${field}][]"]`);
  ...
  private setBusy(busy: boolean): void {
    ...
    this.submitTarget.textContent = busy ? 'Wird gesendet…' : 'Anmelden';
  }
}
