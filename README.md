...
```
tg-freispiel.de/
├── develop.sh                    # Development helper script (starts Webpack + Symfony)
├── assets/
│   ├── images/
│   │   └── stage-background.png  # Original background image used for the site
│   ├── styles/
│   │   └── app.css               # Main CSS (dark theme & glass effects)
│   ├── controllers/              # Stimulus controllers (empty / opt-in)
│   ├── controllers.json          # Stimulus bridge entry
│   └── app.js                    # JavaScript entry (Webpack Encore)
...
│   └── images/                   # Static public images (optional)
├── public/build/                 # Compiled assets (generated by Webpack Encore)
│   ├── images/
...
├── .env                          # Environment variables (committed example)
...
A more detailed breakdown of directories and files follows in the rest of this document.
...
# Create database schemadocker-compose exec web php bin/console doctrine:migrations:migrate --no-interaction
```
